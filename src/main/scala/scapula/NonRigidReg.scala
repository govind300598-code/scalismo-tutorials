...
object NonRigidReg {
  ...
  def registerAll(
    ...
    println("  Building multi-scale GP prior (Cholesky)...")

    val k1 = GaussianKernel[_3D](sigma = 80.0, scaleFactor = 50.0)
    val k2 = GaussianKernel[_3D](sigma = 30.0, scaleFactor = 10.0)
    val k3 = GaussianKernel[_3D](sigma = 10.0, scaleFactor = 2.0)
    val multiScaleKernel = k1 + k2 + k3

    val gp = GaussianProcess[_3D, EuclideanVector[_3D]](DiagonalKernel[_3D](multiScaleKernel, 3))
