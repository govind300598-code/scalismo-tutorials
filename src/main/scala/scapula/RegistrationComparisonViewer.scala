 ...
 *   SCAPULA_REF_ID        - reference specimen id
 *                           (default: paired_scapula_010_F_43_L — lowest-RMS left scapula
 *                            across the paired dataset; falls back to first specimen if absent)
...
object RegistrationComparisonViewer {
  ...
  // Preferred reference: lowest-RMS left scapula in the paired dataset.
  // Both sides of subject 010 (F, 43 y) sit near the population centre
  // (010_F_43_R RMS=1.149 mm, 010_F_43_L RMS=1.289 mm when registered to
  // the former first-alphabetical reference), making this a consistently
  // average shape.  Left side chosen so no mirroring is applied to the
  // reference itself.
  val preferredRefId: String = "paired_scapula_010_F_43_L"
  ...
  def main(args: Array[String]): Unit = {
    ...
    // ---------------------------------------------------------------- reference
    // Priority: SCAPULA_REF_ID env var > preferred default > first alphabetically
    val refSpec = sys.env.get("SCAPULA_REF_ID")
      .flatMap(id => specimens.find(_.modelId == id))
      .orElse(specimens.find(_.modelId == preferredRefId))
