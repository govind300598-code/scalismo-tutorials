...
/** Builds and validates a PCA-based Statistical Shape Model from registered meshes. */
object SSMBuilder {
  ...
  /** Print basic variance/compactness metrics; return the table as a string. */
  ...
  def generalizationError(
    ...
    val errors = meshes.zipWithIndex.map { case (testMesh, i) =>
      ...
      val observations = testMesh.pointSet.pointIds.toIndexedSeq.map(ptId => (ptId, testMesh.pointSet.point(ptId)))
      val posterior    = loo.posterior(observations, 0.5)
      val d            = Metrics.surfaceDistances(testMesh, posterior.mean)
      d.sum / d.length
    }
  ...
  /** Specificity: sample random SSM instances and measure distance to nearest training shape. */
  def specificityError(
    ...
    val errors = (0 until nSamples).map { _ =>
      ...
      val minDist = trainingMeshes.map { ref =>
        val d = Metrics.surfaceDistances(sample, ref); d.sum / d.length
      }.min
      minDist
    }
    val meanError = errors.sum / errors.length
  ...
  final case class SSMMetrics(
    label: String, rank: Int,
    mode1Pct: Double, top5Pct: Double, top10Pct: Double,
    generalization: Double, specificity: Double
  ) {
    def tableRow: String =
      f"| $label%-5s | $mode1Pct%6.1f%% | $top5Pct%6.1f%% | $top10Pct%7.1f%% | $generalization%14.3f | $specificity%11.3f |"
  }
  ...
  def computeMetrics(
    ...
    SSMMetrics(
      label, model.rank,
      pct(0),
      cumPct(math.min(4, cumPct.length - 1)),
      cumPct(math.min(9, cumPct.length - 1)),
      generalizationError(trainingMeshes, label),
      specificityError(model, trainingMeshes, label = label)
    )
  }
  ...
  def writeMetricsTable(metrics: IndexedSeq[SSMMetrics], file: File): Unit = {
    file.getParentFile.mkdirs()
    val header = "| Model | Mode 1 | Top 5 | Top 10 | Generalization | Specificity |"
    val sep    = "|-------|-------:|------:|-------:|---------------:|------------:|"
    val pw = new PrintWriter(file)
    pw.println(header); pw.println(sep)
