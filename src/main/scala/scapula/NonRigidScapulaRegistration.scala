...
object NonRigidScapulaRegistration {
  def main(args: Array[String]): Unit = {
    ...
    // Fix 1: type parameter is [_3D] only — DiagonalKernel's apply takes one D type arg,
    // not [D, EuclideanVector[D]]. Fix 2: pass referenceMesh (DiscreteDomain) not
    // referenceMesh.pointSet (UnstructuredPoints, which is not a DiscreteDomain).
    ...
    def gpIcp(
      ...
      for (i <- 1 to numIter) {
        // Fix 3: posterior expects Point[_3D] as the first tuple element, not PointId.
    ...
    val gridResults: Seq[GridResult] =
      for {
        ...
        val errs: IndexedSeq[Double] = preAligned.map { case (_, tgt) =>
          Metrics.symmetric(gpIcp(gp, tgt, numIter = 15, numSamples = 300), tgt).mean
        }
        val avg = errs.foldLeft(0.0)(_ + _) / errs.size
    ...
    val worstCase = allStats.maxBy { case (_, st) => st.mean }
