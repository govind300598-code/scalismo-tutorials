...
object NonRigidScapulaRegistration extends App {
  ...
  //   Pick the left-side specimen whose 5-landmark centroid is nearest to the
  //   dataset centroid — the most "average" bone, minimising reference bias.
  // ═══════════════════════════════════════════════════════════════════════════════
  println("\n════════════════════════════════════════")
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // GP-ICP helpers (shared by Tasks 2, 3, 4)
  // ═══════════════════════════════════════════════════════════════════════════════

  /** Build a low-rank GP prior on the reference mesh point set. */
  def buildGP(sigma: Double, scale: Double): LowRankGaussianProcess[_3D, EuclideanVector[_3D]] = {
    ...
    val scalarK = GaussianKernel3D(sigma = sigma, scaleFactor = scale)
    val matrixK = DiagonalKernel3D(scalarK, 3)
  ...
  /** Landmark Procrustes + trimmed rigid ICP. */
   ...
   *
   * At each iteration correspondences are found on the current mean-deformed mesh.
   * The posterior is computed from the PRIOR each time (not the previous posterior)
   * so stale noise does not accumulate across iterations.
   */
  def gpIcp(
    ...
    // Start from prior mean; update correspondence search point each iteration.
    ...
    for (i <- 1 to numIter) {
      val observations: IndexedSeq[(PointId, EuclideanVector[_3D], MultivariateNormalDistribution)] =
        sampleIds.map { pid =>
          ...
          // Observation: displacement taking the reference vertex to the target surface.
  ...
  //   sigma ∈ {30, 65, 100} mm  ×  scale ∈ {50, 150}  → 6 combinations
  //   Evaluated on 5 structurally diverse targets (spread across sorted list)
  // ═══════════════════════════════════════════════════════════════════════════════
  println("\n══════════════════════════════════════════════")
  ...
  println("\n  Pre-aligning five targets (landmark + rigid ICP)...")
  ...
  //   Full 40-iter registration with best params; save STL per target; open viewer
  // ═══════════════════════════════════════════════════════════════════════════════
  println("\n════════════════════════════════════════════════════")
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // TASK 4 — Numerical error analysis (7 cases minimum)
  //   1. Reference self-error
  //   2–6. Five diverse targets
  //   7. Worst-performing specimen from up to 30 remaining left-side specimens
  // ═══════════════════════════════════════════════════════════════════════════════
  println("\n══════════════════════════════════════════════════════")
  println("  TASK 4 — Numerical Error Analysis (7 cases)")
  println("══════════════════════════════════════════════════════")

  // 1. Reference self-error (should be ~0)
  ...
  // 7. Worst case in remaining data
  ...
  // ── Open Scalismo viewer for Task 3 visual inspection ────────────────────────
  if (showUi) {
    println("\n  Opening Scalismo viewer — close the window or press ENTER to exit.")
    val ui = ScalismoUI()
