...
object NonRigidScapulaRegistration {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // =========================================================================
    // TASK 1 — Reference Scapula Selection
    // =========================================================================
    println()
    println("=" * 62)
    ...
    // =========================================================================
    // Build Gaussian kernel GP prior  (DiagonalKernel[_3D] — no Value type param)
    // =========================================================================
    def buildGP(sigma: Double, scale: Double): LowRankGaussianProcess[_3D, EuclideanVector[_3D]] = {
      // Zero-mean field over all of 3D Euclidean space
      val zeroMean = Field(EuclideanSpace[_3D], (_: Point[_3D]) => EuclideanVector.zeros[_3D])
      // Gaussian kernel * scale gives the amplitude; DiagonalKernel combines 3 scalar kernels
      val scalarK  = GaussianKernel[_3D](sigma) * scale
      val matrixK  = DiagonalKernel[_3D](scalarK, 3)
    ...
    // =========================================================================
    // RIGID registration: landmark Procrustes + trimmed ICP
    // =========================================================================
    ...
    // =========================================================================
    // NON-RIGID registration: GP-ICP via posterior update
    //
    // Training data uses Point[_3D] (not PointId) as required by the continuous
    // LowRankGaussianProcess.posterior API.  After convergence, a short round of
    // rigid ICP removes any global drift the non-rigid step may have introduced.
    // =========================================================================
    def gpIcp(
      ...
      for (i <- 1 to numIter) {
        // Build training data: (refPoint, targetDisplacement, noise)
        // LowRankGaussianProcess.posterior takes Point[_3D], not PointId
        val trainingData: IndexedSeq[(Point[_3D], EuclideanVector[_3D], MultivariateNormalDistribution)] =
          sampleIds.map { pid =>
            ...
            (refPt, closestPt - refPt, noise)
          }
        val posterior = prior.posterior(trainingData)
        currentMesh   = referenceMesh.transform(pt => pt + posterior.mean(pt))
        if (i % 10 == 0) println(s"      GP-ICP iter $i / $numIter")
      }

      // Round of rigid ICP after non-rigid to remove global drift
      RigidAlign.rigidIcp(currentMesh, target, iterations = 5)
    }

    // =========================================================================
    // TASK 2 — Hyperparameter Grid Search
    // "Seventy and seven": sigma=70, scale=7 is the headline candidate
    // =========================================================================
    println()
    println("=" * 62)
    ...
    println("  Pre-aligning (landmark Procrustes + rigid ICP) ...")
    ...
    // Gaussian kernel grid — sigma=70, scale=7 ("seventy and seven") is first
    val sigmas = Seq(70.0, 30.0, 100.0)
    val scales = Seq(7.0, 70.0, 150.0)
    ...
    val gridResults: Seq[GridResult] =
      for {
        ...
        print(f"    sigma=$sigma%5.0f  scale=$scale%6.1f  ->  ")
    ...
    // =========================================================================
    // TASK 3 — Visual Assessment (40 non-rigid iters, best params)
    // =========================================================================
    println()
    println("=" * 62)
