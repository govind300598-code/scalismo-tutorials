...
object VisualizationApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Mirror right side so all specimens are anatomically left.
    ...
    val rigidAligned: IndexedSeq[Spec] = lmAligned.map { s =>
      ...
      s.copy(mesh = refined)      // landmarks no longer needed after this stage
    }
    ...
    // ── Decimate reference for non-rigid GP registration ──────────────────────
    ...
    // Each pass re-registers all specimens to the mean of the previous pass.
    // Cached .vtk files are loaded from outDir when available.
    ...
    SSMBuilder.loadMeshes(finalTag) match {
      ...
      case None =>
        for (pass <- 1 to Config.refinePasses) {
          println(s"[S0${4 + pass}] Non-rigid registration – Pass $pass (GP-ICP, σ=${Config.gpSigma}mm × ${Config.gpScale})")
    ...
    // Show pass-1 if cached separately.
    ...
    ui.show(s07, currentRef, s"Mean_Pass${Config.refinePasses}")
    ...
    // ── S09–S11: First 3 PCA modes at ±1σ / ±2σ / ±3σ ───────────────────────
    for (modeIdx <- 0 until math.min(3, ssm.rank)) {
      val ev     = eigenValues(modeIdx)
      val sigma  = math.sqrt(ev)
      val varPct = (ev / totalVar * 100).toInt
      println(s"[S${9 + modeIdx}] Mode ${modeIdx + 1}: σ=${f"$sigma%.1f"}mm  var=$varPct%")
