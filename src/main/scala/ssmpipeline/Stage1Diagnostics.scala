...
/**
 * STAGE 1 -- DIAGNOSTICS. Run before anything else.
 *
 * Answers: is the large surface distance caused by a pipeline defect (mirror axis, landmark correspondence, rigid
 * alignment) or by genuine anatomical difference between two people's bones?
 *
 * The design exploits the PAIRED dataset: each subject contributes a left and a right scapula. Within-subject
 * bilateral comparison gives a built-in ground truth that the between-subject comparison does not have.
 */
