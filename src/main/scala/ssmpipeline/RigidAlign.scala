...
object RigidAlign {
   ...
   *
   * Sampling by index stride under-samples thin structures such as the acromion and coracoid process because STL
   * vertex order reflects scan/mesh-generation order, not spatial distribution.
   */
   ...
   *
   * Like every ICP variant this minimises a MEAN criterion and gives no guarantee about the Hausdorff distance.
   */
  def rigidIcp(moving: TriangleMesh[_3D],
    ...
    for (_ <- 0 until iterations) {
      val pairs  = ids.map { id => val p = current.pointSet.point(id); (p, ops.closestPointOnSurface(p).point) }
